...
const icons = {
  ...
  sub:     "M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01",
...
const ORDERS = [
  {
    ...
    items: 6,
    price: 25.00,
    address: "42 Green Lane, Pune, MH 411001",
    products: [
...
export default function MyOrders() {
  ...
  const filtered = ORDERS
    ...
    .sort((a, b) => {
      if (sortBy === "newest")  return ORDERS.indexOf(b) - ORDERS.indexOf(a) < 0 ? 1 : -1;
      if (sortBy === "oldest")  return ORDERS.indexOf(a) - ORDERS.indexOf(b) < 0 ? 1 : -1;
  ...
  const stats = {
    ...
    spent:     ORDERS.reduce((s, o) => s + o.price, 0),
  };
    ...
    <div className="sv-mo__app">
      ...
      <aside className="sv-mo__sidebar">
        ...
        <nav className="sv-mo__nav">
          {[
            ...
            <button
              key={id}
              className={`sv-mo__nav-item${id === "myorders" ? " sv-mo__nav-item--active" : ""}`}
              onClick={() => navigate(path)}
            >
              {iconKeys.map((k, i) => (
                <Icon key={i} d={icons[k]} size={20} />
              ))}
              {label}
            </button>
          ))}
      ...
      <main className="sv-mo__main">
        ...
        <div className="sv-mo__header">
          ...
          <button className="sv-mo__reorder-btn">
            <Icon d={icons.rotate} size={16} />
            Reorder Last Box
          </button>
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className={`sv-mo__order-card${isOpen ? " sv-mo__order-card--expanded" : ""}`}
                >
                  {/* Card Header */}
                    ...
                    <div className="sv-mo__order-details">
                            ...
                            {TRACK_LABELS.map((label, idx) => {
                              ...
                              return (
                                <div key={label} className="sv-mo__track-step">
                                  ...
                                  <div className={`sv-mo__track-label${active ? " sv-mo__track-label--active" : ""}`}>
                                    {label}
                                  </div>
                                </div>
                              );
                      ...
                      <div className="sv-mo__details-grid">
                        ...
                        <div className="sv-mo__items-section">
                          <div className="sv-mo__section-title">Items in This Box</div>
                          {order.products.map((p) => (
                            <div key={p.name} className="sv-mo__item-row">
                              ...
                              <div className="sv-mo__item-price">${p.price.toFixed(2)}</div>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="sv-mo__delivery-section">
                          ...
                          <div className="sv-mo__delivery-info">
                            {[
                              ...
                              ["Status",        badgeLabel[order.status]],
                            ].map(([key, val]) => (
                          ...
                          <div className="sv-mo__details-actions">
                            <button className="sv-mo__detail-btn sv-mo__detail-btn--primary">
                              <Icon d={icons.eye} size={14} /> &nbsp;Track Order
                            </button>
                            <button className="sv-mo__detail-btn sv-mo__detail-btn--outline">
                              <Icon d={icons.rotate} size={14} /> &nbsp;Reorder
                            </button>
                            {order.status !== "cancelled" && order.status !== "delivered" && (
                              <button className="sv-mo__detail-btn sv-mo__detail-btn--danger">
                                Cancel
                              </button>
                            )}
                          </div>
